...
class PreviewWindow(QMainWindow, secondwindow_auto.Ui_SecondWindow):
    def __init__(self,parent = None):
        ...
        self.Browsebutton.clicked.connect(lambda: self.pressedbrowsebutton())
    ...
    def handleimage(self):
        ...
        if self.logo.lower() != 'default':
            mimage = Image.open(self.filename)
            ...
            mimage.save(self.filename)
        self.pic = PyQt5.QtWidgets.QLabel(self)
        self.pic.setScaledContents(True)
        self.pic.setPixmap(PyQt5.QtGui.QPixmap(self.filename))
        self.pic.setGeometry(self.geometry().height()/2,self.geometry().width()/10,320,720)
        self.pic.show()
